...
if (isset($_GET["usersitenumber"]) && isset($_GET["usernumber"]) && is_numeric($_GET["usersitenumber"]) &&
    is_numeric($_GET["usernumber"]) && isset($_GET["confirmation"])) {
    if($_GET["confirmation"]=="active"){
        if(!enabledUser($_SESSION["usertable"]["contestnumber"],$_GET["usersitenumber"],$_GET["usernumber"]))
    ...
    if($_GET["confirmation"]=="delete"){
        if (!DBDeleteUser($_SESSION["usertable"]["contestnumber"], $_GET["usersitenumber"], $_GET["usernumber"]))
...
if (isset($_POST["username"]) && isset($_POST["userfullname"]) && isset($_POST["userdesc"]) && isset($_POST["userip"]) &&
    isset($_POST["usernumber"]) && isset($_POST["usersitenumber"]) && isset($_POST["userenabled"]) &&
...
for ($i=0; $i < count($usr); $i++) {
      echo " <tr>\n";
      if(($usr[$i]["usersitenumber"] == $_SESSION["usertable"]["usersitenumber"]) &&
	   //$usr[$i]["usertype"] != 'site' &&
      ...
      if ($usr[$i]["usersession"] != "")
        echo "  <td><a href=\"javascript: conf2('user.php?logout=1&site=" . $usr[$i]["usersitenumber"] . "&user=" .
      ...
      //if(strpos($prob[$i]["fullname"],"(DEL)") !== false) {
        //  echo "<div class=\"btn-group btn-group-toggle\" data-toggle=\"buttons\"><a href=\"javascript: conf3('problem.php?delete=" . $prob[$i]["number"] . "&input=" . myrawurlencode($prob[$i]["inputfilename"]) .
        //       "')\" class=\"btn btn-warning\">Recuperar</a>";
        //       echo "<button type=\"button\" onclick=\"mostrar(".$prob[$i]["number"].")\" class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"#subproblem\" name=\"".$prob[$i]["number"]."\" id=\"".$prob[$i]["number"]."\" disabled>Actulizar</button></div>";
       //} else {
        //   echo "<div class=\"btn-group btn-group-toggle\" data-toggle=\"buttons\"><a href=\"javascript: conf2('problem.php?delete=" . $prob[$i]["number"] . "&input=" . myrawurlencode($prob[$i]["inputfilename"]) .
        //       "')\" class=\"btn btn-danger\">Eliminar</a>";
          // echo "<button type=\"button\" onclick=\"mostrar(".$prob[$i]["number"].")\" class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"#subproblem\" name=\"".$prob[$i]["number"]."\" id=\"".$prob[$i]["number"]."\">Actulizar</button></div>";
       //}
       //echo "  <td>" . "ELIMINAR ACTUALIZAR" . "&nbsp;</td>\n";
       if(($usr[$i]["usersitenumber"] == $_SESSION["usertable"]["usersitenumber"]) &&
 	       $usr[$i]["usertype"] != 'site' &&
 	      ($usr[$i]["usernumber"] != $_SESSION["usertable"]["usernumber"] ||
 	      $usr[$i]["usersitenumber"] != $_SESSION["usertable"]["usersitenumber"])){
            ...
            if($usr[$i]['userenabled'] != 't' && $usr[$i]['userlastlogin'] < 1){
                 echo " <td><div class=\"btn-group btn-group-toggle\" data-toggle=\"buttons\"><a onClick=\"conf7(".$usr[$i]["usersitenumber"].",".$usr[$i]["usernumber"].")\"" .
                 ...
                 echo " <td><div class=\"btn-group btn-group-toggle\" data-toggle=\"buttons\"><a " .
                     "')\" class=\"btn btn-danger\" onClick=\"conf4(".$usr[$i]["usersitenumber"].",".$usr[$i]["usernumber"].")\">Eliminar</a>";
                 echo "<a href=\"user.php?site=" . $usr[$i]["usersitenumber"] . "&user=" .
        		  $usr[$i]["usernumber"] . "#form_user\" class=\"btn btn-primary\" name=\"\" >Actualizar</a></div>";
            }
            echo "<script language=\"javascript\">    function conf4(site, user) {\n";
            ...
            echo "            document.location='user.php?usersitenumber='+site+'&usernumber='+user+'&confirmation=delete';\n";
            echo "      }\n";
            echo "    }</script>\n";
            echo "<script language=\"javascript\">    function conf7(site, user) {\n";
            echo "      if (confirm('ESTAS SEGURO DE ACTIVAR USUARIO?')) {\n";
            //echo "            document.location='https://www.google.com/?hl=es'\n";
            echo "            document.location='user.php?usersitenumber='+site+'&usernumber='+user+'&confirmation=active';\n";
            echo "      }\n";
            echo "    }</script>\n";
          //echo "  <td><a href=\"user.php?site=" . $usr[$i]["usersitenumber"] . "&user=" .
...
<div class="container">
    ...
    ?>
      ...
      <!--<div class="form-group row">
          <label for="usersitenumber" class="col-sm-4 col-form-label">User Site Number:</label>
          <div class="col-sm-8">-->
              <input type=hidden class="form-control" name="usersitenumber" id="usersitenumber" <?php echo "value=\"" . $usite . "\""; ?> maxlength="20" />
          <!--</div>
      </div>-->
      <div class="form-group row">
